		  ...
		  uint16_t protocol_version)
{
	...
	printf("OK, I am going to de slurm cred sign!\n");
	if (_slurm_cred_sign(ctx, cred, protocol_version) < 0) {
		printf("OK, I am eventually failed!\n");
		goto fail;
	}

	slurm_mutex_unlock(&ctx->mutex);
	slurm_mutex_unlock(&cred->mutex);
	printf("OK, I am succeed!\n");
	return cred;
			...
			uint32_t *job_mem_limit, uint32_t *step_mem_limit)
{
	...
	*job_mem_limit = cred->job_mem_limit & (~MEM_PER_CPU);
	...
	debug3("testing 6, host index=%d", host_index);
#endif
