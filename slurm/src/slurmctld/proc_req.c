...
static void _slurm_rpc_allocate_resources(slurm_msg_t * msg)
{
	...
	if (!slurm_get_peer_addr(msg->conn_fd, &resp_addr)) {
		...
		if (error_code == SLURM_SUCCESS) {
			...
			//printf("After allocating resources!\n");
			insert_job_record(a_job_res);
...
//		job_waiting = true;
	...
	if ((error_code == SLURM_SUCCESS)) {//|| ((immediate == 0) && job_waiting)) {
		//xassert(job_ptr);
...
//		     job_ptr->nodes, TIME_STR);
		...
		info("sched: _slurm_rpc_allocate_resources JobId=%u "
		     "NodeList=%s %s",a_job_res->job_id,
		     a_job_res->nodelist, TIME_STR);
...
//		}
		alloc_msg.num_cpu_groups = 1;
		alloc_msg.cpu_count_reps = xmalloc(sizeof(uint32_t) * 1);
		*(alloc_msg.cpu_count_reps) = a_job_res->num_node;
		alloc_msg.cpus_per_node = xmalloc(sizeof(uint16_t) * a_job_res->num_node);
		int i;
		for (i = 0; i < a_job_res->num_node; i++)
			alloc_msg.cpus_per_node[i] = 2;
		alloc_msg.error_code     = error_code;
		alloc_msg.job_id         = a_job_res->job_id; //job_ptr->job_id;
		alloc_msg.node_cnt       = a_job_res->num_node; //job_ptr->node_cnt;
		alloc_msg.node_list      = xstrdup(a_job_res->nodelist);//(job_ptr->nodes);
		alloc_msg.partition      = xstrdup("default");//xstrdup(job_ptr->partition);
		alloc_msg.alias_list     = xstrdup("kodiak");//xstrdup(job_ptr->alias_list);
		alloc_msg.select_jobinfo =
			select_g_select_jobinfo_copy(job_desc_msg->select_jobinfo);//(job_ptr->select_jobinfo);
		select_g_select_jobinfo_set(alloc_msg.select_jobinfo,
					    SELECT_JOBDATA_USER_NAME,
					    &job_desc_msg->user_id);
		select_g_select_jobinfo_set(alloc_msg.select_jobinfo,
					    SELECT_JOBDATA_NETWORK,
					    job_desc_msg->network);
		alloc_msg.select_jobinfo =
					select_g_select_jobinfo_copy(alloc_msg.select_jobinfo);
//		if (job_ptr->details) {
...
//		}
	...
	//insert_job_metadata_zht(a_job_res);
...
static void _slurm_rpc_complete_prolog(slurm_msg_t * msg)
{
	...
	error_code = SLURM_SUCCESS;//prolog_complete(comp_msg->job_id,
...
static void _slurm_rpc_job_step_create(slurm_msg_t * msg)
{
	...
	if (error_code == SLURM_SUCCESS) {
		...
		slurm_cred_arg_t *cred_arg = xmalloc(sizeof(slurm_cred_arg_t));
		memset(cred_arg, 0, sizeof(slurm_cred_arg_t));
		//memset(&cred_arg, 0, sizeof(slurm_cred_arg_t));

		cred_arg->jobid    = req_step_msg->job_id;
		cred_arg->stepid   = step_rec->step_id;
		cred_arg->uid      = uid;

		int i;
		cred_arg->job_core_bitmap = bit_alloc(req_step_msg->min_nodes);
		//xmalloc(sizeof(bitstr_t) * req_step_msg->min_nodes);
//		for (i = 0; i < req_step_msg->min_nodes; i++) {
//			cred_arg->job_core_bitmap[i] = 2;
//		}
		//cred_arg.job_core_bitmap = NULL;//job_resrcs_ptr->core_bitmap;
		cred_arg->job_core_spec   = 0;//job_ptr->details->core_spec;
		cred_arg->job_hostlist    = req_step_msg->node_list;//step_rec->host;//job_resrcs_ptr->nodes;
		cred_arg->job_mem_limit   = 0;//job_ptr->details->pn_min_memory;
		cred_arg->job_nhosts      = req_step_msg->min_nodes;//job_resrcs_ptr->nhosts;
		cred_arg->job_gres_list   = step_rec->gres_list;//job_ptr->gres_list;
		cred_arg->step_gres_list  = step_rec->gres_list;
		cred_arg->step_core_bitmap = bit_alloc(req_step_msg->min_nodes);
		//xmalloc(sizeof(bitstr_t) * req_step_msg->min_nodes);
//		for (i = 0; i < req_step_msg->min_nodes; i++) {
//			cred_arg->step_core_bitmap[i] = 2;
//		}
		//cred_arg.step_core_bitmap = NULL;//step_ptr->core_bitmap_job;
	#ifdef HAVE_FRONT_END
		xassert(job_ptr->batch_host);
		cred_arg.step_hostlist   = job_ptr->batch_host;
	#else
		cred_arg->step_hostlist   = req_step_msg->node_list;//step_rec->host;
	#endif
		//if (step_ptr->pn_min_memory)
			cred_arg->step_mem_limit  = 0;//step_ptr->pn_min_memory;

		cred_arg->cores_per_socket    = xmalloc(sizeof(uint16_t) * req_step_msg->min_nodes);
		for (i = 0; i < req_step_msg->min_nodes; i++) {
			cred_arg->cores_per_socket[i] = 2;
		}
		//cred_arg.cores_per_socket    = 0;//job_resrcs_ptr->cores_per_socket;
		cred_arg->sockets_per_node  	 = xmalloc(sizeof(uint16_t) * req_step_msg->min_nodes);
		for (i = 0; i < req_step_msg->min_nodes; i++) {
			cred_arg->sockets_per_node[i] = 1;
		}
		//cred_arg.sockets_per_node    = 0;//job_resrcs_ptr->sockets_per_node;

		cred_arg->sock_core_rep_count = xmalloc(sizeof(uint32_t) * req_step_msg->min_nodes);
		for (i = 0; i < req_step_msg->min_nodes; i++) {
			cred_arg->sock_core_rep_count[i] = 1;
		}
		//cred_arg.sock_core_rep_count = 0;//job_resrcs_ptr->sock_core_rep_count;
...
static void _slurm_rpc_node_registration(slurm_msg_t * msg)
{
	...
	/* Use ZHT to store all the resource info (key, value) :
	 * (self_id, {part_size, node-1, node-2, ...,})
	 * */
	int num_insert_msg_local = 0;

	pthread_mutex_lock(&regist_mutex);
	if (find_exist(source, node_reg_stat_msg->node_name, num_regist_recv) < 0) {

		strcpy(source[num_regist_recv++], node_reg_stat_msg->node_name);
