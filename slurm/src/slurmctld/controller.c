...
pthread_mutex_t num_job_recv_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static void _parse_commandline(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "BcC:dDf:hi:l:L:m:M:n:p:rRu:vVz:Z:")) != -1)
		...
		case 'c':
			...
			bg_recover = 0;
			break;
		case 'C':	// number of controller
			num_ctrl = strtol(optarg, &tmp_char, 10);
			break;
		case 'd':
			daemonize = 1;
			break;
		...
		case 'p':	// partition size
			part_size = strtol(optarg, &tmp_char, 10);
			break;
		case 'r':
			recover = 1;
		...
		case 'V':
			...
			exit(0);
			break;
		case 'z':	// ZHT configure file
			zht_cfg_file = xstrdup(optarg);
			break;
		case 'Z':	// ZHT member file
			zht_mem_file = xstrdup(optarg);
			break;
		default:
			_usage(argv[0]);
			exit(1);
