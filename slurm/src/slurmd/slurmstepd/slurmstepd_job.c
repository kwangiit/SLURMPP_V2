...
stepd_step_rec_create(launch_tasks_request_msg_t *msg)
{
	...
	if (!_valid_uid_gid((uid_t)msg->uid, &(msg->gid), &(msg->user_name))) {
		debug3("Failue uid gid");
		return NULL;
	}
	debug3("entering stepd_step_rec_create1");

	if (acct_gather_check_acct_freq_task(msg->job_mem_lim, msg->acctg_freq)) {
		debug3("Failure check acct freq!");
		return NULL;
	}

	debug3("entering stepd_step_rec_create2");
	...
	debug3("entering stepd_step_rec_create3");
	...
	*/
	debug3("entering stepd_step_rec_create4");
	acct_gather_profile_g_node_step_start(job);

	debug3("entering stepd_step_rec_create5");
	acct_gather_profile_startpoll(msg->acctg_freq,
				      conf->job_acct_gather_freq);

	debug3("entering stepd_step_rec_create6");
	...
	debug3("entering stepd_step_rec_create9");
	...
	debug3("entering stepd_step_rec_create10");

	_job_init_task_info(job, msg->global_task_ids,
			    msg->ifname, msg->ofname, msg->efname);

	debug3("entering stepd_step_rec_create11");
	return job;
}
